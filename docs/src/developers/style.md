...
!!! info
    The style guide is always a work in progress, and not all MappingUtils code
    follows the rules. When modifying MappingUtils, please fix the style violations
...
`MappingUtils` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/MappingUtils.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `MappingUtils` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `MappingUtils` have
