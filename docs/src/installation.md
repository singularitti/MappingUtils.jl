...
[`MappingUtils`](https://github.com/singularitti/MappingUtils.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("MappingUtils")
   ```

3. Run

   ```julia-repl
   julia> using MappingUtils
   ```
...
version of `MappingUtils`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/MappingUtils.jl"
```
...
our [GitHub repository](https://github.com/singularitti/MappingUtils.jl)
for new releases.
Once we release a new version, you can update `MappingUtils` by typing

```@repl
using Pkg
Pkg.update("MappingUtils")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `MappingUtils`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("MappingUtils")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `MappingUtils`.
